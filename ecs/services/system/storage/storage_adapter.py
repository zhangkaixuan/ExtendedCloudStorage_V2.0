...
from ecs.util.params import SWIFT_AUTH_URL, SWIFT_USER, SWIFT_PW
...
class ApiMapping():
    ...
    def _api_mapping(self, fun_name, api_type, **kws):
        ...
        apis = minidom.parse("/etc/ecs/apiconfig.xml")

        for api in apis.getElementsByTagName('api'):
            if api.attributes['name'].firstChild.nodeValue == fun_name:

                swift_api_name = api.getElementsByTagName(api_type)[0].getElementsByTagName('name')[
                ...
                return getattr(swift, swift_api_name)(**param_dict)
                break
...
def test():
    dic = {"auth_url": SWIFT_AUTH_URL, 'user': SWIFT_USER, 'key': SWIFT_PW, 'domain_name': 'admin'}
