...
def get_keystoneclient_2_0(auth_url, user, key, os_options):
    ...
    """
    from api.keystone import client as ksclient
    from api.keystone import exceptions
...
def get_auth(url, user, key, tenant_name=None):
    url = url
    body = {'auth': {'passwordCredentials': {'password': key,
                                             'username': user}, 'tenantName': tenant_name}}
...
class Connection(object):
    ...
    def __init__(self, authurl, user, key, tenant_name=None):
        ...
        self.attempts = 0
        self.snet = False
        self.starting_backoff = 1
        self.auth_version = "1"
        self.os_options = None or {}
        if tenant_name:
...
def test():
    ...
    print "测试swift的各个接口"
    print "*"*30
    conn = Connection(authurl='http://192.168.1.108:5000/v2.0/', user='admin', key='ADMIN', tenant_name='admin')
    print "*"*30
    print "认证：user:admin, pw:ADMIN, tenant:admin"
    print "*"*30
    ...
    print "*"*30
    print "创建一个容器叫做test_container"
    print "*"*30
    print conn.put_container("test_container")
    print "*"*30
    print "获取容器test_container"
    print "*"*30
    print conn.get_container("test_container")
    print "*"*30
    print "在test_container下创建一个文件test"
    print "*"*30
    file = open("/home/zhangkaixuan/test")
    conn.put_object('test_container', 'this is a test file', file, headers={})
    headers,body = conn.get_container('test_container')
    print "*"*30
    print "获取test_container下所有的object"
    print "*"*30
    for obj in body:
        print obj
    print "*"*30
    # print "获取object,路径：/test_container/test"
    # print "*"*30
    # object = conn.get_object('test_container', 'test')
    # print object



#    conn.head_container("AFDSA")
#print conn.head_container("AFDSA")
#print conn.head_account()
#    file = open('C:\\Users\\Adrian\\Downloads\\x.jpg','wb')
#    aa = unpack(service.downloadAttachment(token,id))
#    file.write(conn.get_object('adrian','fasdfasf')[1])
#    file.close()
#    conn.put_container('nihaoya')
#    conn.delete('adrian','你好')

#    print conn.get_container('haha')[1]
#    print [(n['name'],n['bytes']) for n in conn.get_container('haha')[1]]

#    x = open(filepath,'rb')
###    x = x.read()
###    x = conn.get_object('adrian', 'ddd')
###    print x
#    (filepath,filename)=os.path.split(x.name)
#    conn.put_object('adrian',filename,x,headers = {'x-object-meta-orig-filename':filename})
#    x.close()
##    url = 'http://192.168.1.105:5000/v2.0/'
##    body = {'auth':    {'passwordCredentials': {'password': 'ADMIN',
##        'username':'ADMIN'}}}
##    token_url = urljoin(url, "tokens") 
##    resp, body = json_request("POST", token_url, body=body)
##    print resp,body
##    print conn.head_object('adrian', 'xxx')
##    print conn.head_object('adrian', 'wow')
